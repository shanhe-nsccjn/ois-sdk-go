...
用于对用户存储于 OIS 对象存储上的图片进行各种基本处理，例如格式转换，裁剪，翻转，水印等。
...
具体文档说明参考 [API Docs](https://docsv3.shanhe.com/ois/data_process/image_process) 。
...
`operation_param`是图像操作参数，它在`ois-sdk-go / client / image / image.go`中定义。
```go
