# OIS Image Processing Usage Guide

For processing the image stored in OIS by a variety of basic operations, such as format, crop, watermark and so on.
...
Please see [OIS Image API](https://docsv3.shanhe.com/ois/data_process/image_process).
...
import (
	...
	"github.com/shanhe-nsccjn/ois-sdk-go/v4/client/image"
...
Create configuration from Access Key and Initialize the OIS service with a configuration.
```go
// Initialize the OIS service with a configuration
config, _ := config.New("ACCESS_KEY_ID", "SECRET_ACCESS_KEY")
service, _ := qs.Init(config)
```
Initialize a OIS bucket.
```go
...
`operation_param` is the image operation param, which definined in `ois-sdk-go/client/image/image.go`.
```go
...
// *qs.ImageProcessOutput: github.com/shanhe-nsccjn/ois-sdk-go/v4/service/object.go
