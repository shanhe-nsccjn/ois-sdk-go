...
// unpacker is the response unpacker for OIS service.
...
func (b *unpacker) exposeStatusCode(ctx context.Context) error {
	...
	if value.IsValid() {
		...
		case *int:
			logger.Info(
				log.String("title", "OIS response code"),
...
func (b *unpacker) parseResponseHeaders(ctx context.Context) error {
	...
	logger := log.FromContext(ctx)
	logger.Info(
		log.String("title", "OIS response header"),
		log.Int("date", convert.StringToTimestamp(b.resp.Header.Get("Date"), convert.RFC822)),
...
func (b *unpacker) parseResponseBody(ctx context.Context) error {
	...
	if value.IsValid() {
		...
		case "string":
			...
			logger.Info(
				log.String("title", "OIS response body"),
				log.Int("date", convert.StringToTimestamp(b.resp.Header.Get("Date"), convert.RFC822)),
...
func (b *unpacker) parseError(ctx context.Context) error {
	...
	qsError := &errors.OISError{
		StatusCode: b.resp.StatusCode,
		RequestID:  requestID,
	}

	// OIS nginx could refuse user's request directly and only return status code.
