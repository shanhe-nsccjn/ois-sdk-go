...
import (
	...
	"github.com/shanhe-nsccjn/ois-sdk-go/v4"
...
// Builder is the request builder for OIS service.
type Builder struct {
	parsedURL        string
...
func (qb *Builder) BuildHTTPRequest(ctx context.Context, o *data.Operation, i *reflect.Value) (*http.Request, error) {
	...
	logger.Info(
		log.String("title", "Built OIS request"),
		log.Int("date", timestamp),
		log.String("url", httpRequest.URL.String()),
	)

	logger.Info(
		log.String("title", "OIS request headers"),
		log.Int("date", timestamp),
	...
	if qb.parsedBodyString != "" {
		logger.Info(
			log.String("title", "OIS request body string"),
			log.Int("date", timestamp),
...
func (qb *Builder) setupHeaders(httpRequest *http.Request) error {
	...
	if httpRequest.Header.Get("User-Agent") == "" {
		...
		ua := fmt.Sprintf(`ois-sdk-go/%s (%s; %s)`, sdk.Version, version, system)
